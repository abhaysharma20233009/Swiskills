  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        // console.log(profileData.profile.experience);
        // setSkills(response.data.data.data.skills)
        // if (data.status === 'success' && data.data) {
        //   const profileInfo = data.data.data.profile || {};
    ...
    <div className="max-w-4xl mx-auto p-6 font-sans bg-zinc-800">
      ...
      <div className="text-center mb-8">
        ...
        <div className="relative border border-gray-400 rounded-md">
          <img className="w-full h-52 object-cover rounded-lg" src={bannerImage} alt="Banner" />
          <div className="relative -mt-16 flex justify-center">
            <img className="w-32 h-32 rounded-full border-4 border-white shadow-lg" src={profileImage} alt="Profile" />
          </div>
        </div>
