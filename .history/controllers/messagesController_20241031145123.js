...
exports.updateMessage = catchAsync(async (req, res, next) => {
    ...
    if (!chat) {
        return next(new AppError('Message not found or you are not authorized to update this message.', 404));
    }
    ...
    if (messageTime < fifteenMinutesAgo) {
        return next(new AppError('You can only edit a message within 15 minutes', 400));
    }
...
exports.deleteMessage = catchAsync(async (req, res, next) => {
    ...
    // Remove the message
    if (req.user._id.equals(chat.participants.sender)) {
        // Sender deletes the message for both
        await ChatMessage.deleteOne({ _id: chat._id }); // Delete entire chat
        return res.status(204).send(); // No content
    } else {
        // Recipient deletes the message only for themselves
