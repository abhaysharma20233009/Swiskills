...
import ExploreCard from './ExploreCard'; // Ensure this component is imported
...
function SkillModal({ closeModal, addSkill }) {
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      ...
      try {
        ...
        // Assuming your data structure is like { data: { data: { data: [] } } }
        if (response.data && response.data.data && response.data.data.data) {
          setSkills(response.data.data.data); // Set the skills data
        } else {
          setSkills([]); // If the structure is not as expected, clear skills
        }
      } catch (err) {
  ...
  // Filter and sort cards based on the search term
      ...
      <div className="modal-content bg-white rounded-lg p-6 w-96 relative">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
            {loading ? (
              <div className="text-gray-500">Loading...</div> // Show loading text while fetching
            ) : error ? (
              ...
              paginatedSkills.length > 0 ? (
                paginatedSkills.map((card) => (
                  <div key={card.id} onClick={() => toggleSkillSelection(card.name)} className="cursor-pointer">
                    ...
                    />
                    {selectedSkills.includes(card.name) && <FaCheckCircle className="text-green-500" />} {/* Indicate selected skills */}
                  </div>
                ))
              ) : (
                <div className="text-gray-500">No skills found.</div> // Display message if no skills match
              )
            )}
          </div>
        </div>
