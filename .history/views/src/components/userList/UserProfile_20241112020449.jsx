...
function UserProfile({ initialConnectionStatus }) {
  ...
  useEffect(() => {
    if (sentRequests.length > 0) {
      ...
      if (userRequest) {
        setConnectionStatus(userRequest.status); // Dynamically set the connection status based on the request
      }
    }
  }, [sentRequests, userId]);
  ...
  const { name, title, location, about } = profile || {};
  ...
  const handleAcceptConnection = () => {
    setConnectionStatus('accepted'); // Change status to 'accepted' when accepted
  };
    ...
    <div className="max-w-4xl mx-auto p-6 font-sans bg-zinc-800">
      <div className="text-center mb-8">
        ...
        <div className="flex justify-center space-x-4 mt-4">
          ...
          {/* Only render Connection Status and Connect Button if a request has been sent */}
          {sentRequests.length > 0 && (
            <div className="flex items-center space-x-2">
              ...
              {/* Show Connect button only if the status is "pending" or "rejected" */}
              {connectionStatus === 'pending' || connectionStatus === 'rejected' ? (
                <button
                  onClick={handleConnectClick}
                  className="bg-blue-500 text-white py-1 px-2 rounded-md flex items-center"
                >
                  <FaUserPlus className="mr-2" /> {connectionStatus === 'pending' ? 'Reconnect' : 'Connect'}
                </button>
              ) : (
                <button
                  onClick={handleAcceptConnection}
                  className="bg-green-500 text-white py-1 px-2 rounded-md flex items-center"
                >
